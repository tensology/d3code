...
import { createIdeTerminalContract, renderIdeTerminalContract } from "../src/d3/ide-terminal.js"
...
test("IDE terminal contract names PowerTerm buffer, UOPY limits, and live proof", () => {
  const contract = createIdeTerminalContract({ name: "prod", type: "ssh", host: "d3.example", username: "d3", account: "SALES", sessionMode: "persistent", promptPattern: ">" })
  ...
  assert.match(renderIdeTerminalContract(contract), /D3 IDE Terminal Contract/)
  assert.match(renderIdeTerminalContract(contract), /PowerTerm-style screen buffer/)
  assert.match(renderIdeTerminalContract(contract), /Send Policy/)
  assert.match(renderIdeTerminalContract(contract), /Command Plan/)
  assert.match(renderIdeTerminalContract(contract), /D3 terminal definition parity/)
})

test("connector strategy makes PTY primary and UOPY optional for D3 IDE work", () => {
  ...
  assert.ok(strategy.ideRequirements.some((entry) => entry.includes("TERM/define-terminal")))
