...
function html(): string {
  return [
    ...
    "</div>",
    "<div id=\"profileModal\" class=\"modal-backdrop\"><div id=\"profileForm\" class=\"profile-form\"><div><div class=\"menu-title\" id=\"profileFormTitle\">Add profile</div><div class=\"menu-muted\">Configure how D3 Code enters a D3 account.</div></div><div class=\"profile-form-grid\"><label class=\"field\"><span>Name</span><input id=\"profileName\" /></label><label class=\"field\"><span>Type</span><select id=\"profileType\"><option value=\"local\">Local</option><option value=\"ssh\">SSH</option></select></label><label class=\"field\"><span>Host</span><input id=\"profileHost\" /></label><label class=\"field\"><span>User</span><input id=\"profileUser\" /></label><label class=\"field\"><span>D3 account</span><input id=\"profileAccount\" placeholder=\"DM\" /></label><label class=\"field\"><span>Session</span><select id=\"profileSession\"><option value=\"oneshot\">One shot</option><option value=\"persistent\">Persistent</option></select></label><label class=\"field wide\"><span>Entry command</span><input id=\"profileEntry\" /></label><label class=\"field\"><span>Prompt</span><input id=\"profilePrompt\" placeholder=\">\" /></label><label class=\"field\"><span>Safety</span><select id=\"profileSafety\"><option value=\"ask\">Ask</option><option value=\"plan\">Plan</option><option value=\"trust\">Trust</option></select></label></div><div class=\"profile-form-actions\"><button id=\"saveProfile\">Save profile</button><button id=\"cancelProfile\">Cancel</button></div></div></div>",
    "<script>",
    ...
    "function defaultProfile(){return {name:'new-profile',type:'local',account:'DM',entryCommand:'',promptPattern:'>',sessionMode:'oneshot',safetyDefault:'ask'}}",
    "function setValue(id,value){$(id).value=value||''}",
    "function openProfileForm(profile){const p=profile||defaultProfile();$('profileFormTitle').textContent=profile?'Edit profile':'Add profile';$('profileModal').classList.add('open');$('profileMenu').classList.remove('open');setValue('profileName',p.name);setValue('profileType',p.type||'local');setValue('profileHost',p.host);setValue('profileUser',p.username);setValue('profileAccount',p.account);setValue('profileEntry',p.entryCommand);setValue('profilePrompt',p.promptPattern||'>');setValue('profileSession',p.sessionMode||'oneshot');setValue('profileSafety',p.safetyDefault||'ask')}",
    "function closeProfileForm(){$('profileModal').classList.remove('open')}",
    "function collectProfileForm(){const type=$('profileType').value;const profile={name:$('profileName').value.trim(),type,account:$('profileAccount').value.trim()||undefined,entryCommand:$('profileEntry').value.trim()||undefined,promptPattern:$('profilePrompt').value.trim()||undefined,sessionMode:$('profileSession').value,safetyDefault:$('profileSafety').value};if(type==='ssh'){profile.host=$('profileHost').value.trim()||undefined;profile.username=$('profileUser').value.trim()||undefined}return profile}",
