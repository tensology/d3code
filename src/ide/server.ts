...
function html(): string {
  return [
    ...
    ".top{height:44px;display:grid;grid-template-columns:220px 1fr auto;align-items:center;border-bottom:1px solid var(--line);background:var(--chrome);padding:0 12px;gap:14px}.brand{font-weight:760;font-size:14px}.meta{font-size:12px;color:var(--soft);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.profile-wrap{position:relative}.profile-button{display:flex;align-items:center;gap:8px;min-width:156px;justify-content:space-between}.profile-menu{position:absolute;right:0;top:38px;width:320px;background:var(--panel);border:1px solid var(--line);box-shadow:0 18px 42px rgba(0,0,0,.28);border-radius:8px;padding:12px;z-index:10;display:none}.profile-menu.open{display:grid;gap:10px}.profile-row{display:grid;grid-template-columns:1fr auto auto;gap:6px}.menu-title{font-size:12px;font-weight:760}.menu-muted{font-size:11px;color:var(--muted);line-height:1.4}",
    ...
    "<div class=\"top\"><div class=\"brand\">D3 Code IDE</div><div class=\"meta\" id=\"meta\">No profile selected</div><div class=\"profile-wrap\"><button id=\"profileMenuButton\" class=\"profile-button\"><span id=\"profileButtonLabel\">Profiles</span><span>v</span></button><div id=\"profileMenu\" class=\"profile-menu\"><div><div class=\"menu-title\">Profiles</div><div class=\"menu-muted\">Switch D3 connection profiles and set your IDE theme.</div></div><select id=\"profileSelect\"></select><div class=\"profile-row\"><button id=\"useProfile\">Use profile</button><button id=\"editProfile\">Edit</button><button id=\"removeProfile\">Remove</button></div><button id=\"refreshStatus\">Refresh profiles</button><button id=\"themeToggle\">Use light mode</button><div id=\"profileList\" class=\"list mini\"></div></div></div></div>",
    "<div class=\"layout\">",
    ...
    "$('themeToggle').onclick=()=>{const next=document.body.dataset.theme==='dark'?'light':'dark';document.body.dataset.theme=next;$('themeToggle').textContent=next==='dark'?'Use light mode':'Use dark mode'};",
    "let profileCache=[];",
    "$('editProfile').onclick=async()=>{const current=profileCache.find(p=>p.name===$('profileSelect').value);if(!current)return show('profileList',{error:'Select a profile to edit.'});const raw=prompt('Edit profile JSON',JSON.stringify(current,null,2));if(!raw)return;show('profileList',await api('/api/profile/manage',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({profile:JSON.parse(raw),confirmed:true})}));await boot();};",
    "$('removeProfile').onclick=async()=>{const profile=$('profileSelect').value;if(!profile)return show('profileList',{error:'Select a profile to remove.'});if(!confirm(`Remove profile ${profile}?`))return;show('profileList',await api('/api/profile/manage',{method:'DELETE',headers:{'content-type':'application/json'},body:JSON.stringify({profile,confirmed:true})}));await boot();};",
    "async function boot(){const status=await api('/api/status');$('meta').textContent=status.profile?`Profile ${status.profile} / Account ${status.account||'none'}`:'No D3 profile selected';$('profileButtonLabel').textContent=status.profile||'Profiles';$('agentSafety').textContent=`Safety: ${status.safety||'ask'} / Mode: ${status.mode||'chat'}`;const profiles=await api('/api/profiles');profileCache=profiles.profiles||[];$('profileSelect').innerHTML=profileCache.map(p=>`<option value=\"${p.name}\" ${p.name===status.profile?'selected':''}>${p.name} (${p.account||'no account'})</option>`).join('');show('profileList',profiles)}",
