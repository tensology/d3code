...
function html(): string {
  return [
    ...
    ".top{height:44px;display:grid;grid-template-columns:220px 1fr auto;align-items:center;border-bottom:1px solid var(--line);background:var(--chrome);padding:0 12px;gap:14px}.brand{font-weight:760;font-size:14px}.meta{font-size:12px;color:var(--soft);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.profile-wrap{position:relative}.profile-button{display:flex;align-items:center;gap:8px;min-width:156px;justify-content:space-between}.profile-menu{position:absolute;right:0;top:38px;width:320px;background:var(--panel);border:1px solid var(--line);box-shadow:0 18px 42px rgba(0,0,0,.28);border-radius:8px;padding:12px;z-index:10;display:none}.profile-menu.open{display:grid;gap:10px}.profile-row{display:grid;grid-template-columns:1fr auto auto;gap:6px}.theme-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px}.theme-choice.active{background:var(--accent);border-color:var(--accent);color:#fff}.menu-title{font-size:12px;font-weight:760}.menu-muted{font-size:11px;color:var(--muted);line-height:1.4}",
