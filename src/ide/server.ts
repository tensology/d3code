...
function html(): string {
  ...
  return [
    ...
    "async function refreshModels(provider,current){const data=await api('/api/models?provider='+encodeURIComponent(provider));const models=data.models||[];$('modelChoice').innerHTML=models.map(m=>`<option value=\"${escapeHtml(m)}\">${escapeHtml(m)}</option>`).join('');const parsed=modelParts(current||'');const model=parsed.provider===provider?parsed.model:(models[0]||'');if(model&&models.includes(model))$('modelChoice').value=model;$('modelExact').value=model||'';$('modelMenuNote').textContent=data.warning||''}",
    ...
    "$('modelChoice').onchange=()=>{$('modelExact').value=$('modelChoice').value};",
    "$('saveModelChoice').onclick=async()=>{const provider=$('modelProvider').value;const exact=$('modelExact').value.trim()||$('modelChoice').value;const providerIds=[...$('modelProvider').options].map(o=>o.value);const first=exact.split('/')[0];const model=providerIds.includes(first)?exact:provider+'/'+exact;const status=await api('/api/config/model',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model})});await applyStatus(status);closeControlMenus();toast('Model set to '+status.model)};",
...
async function route(req: IncomingMessage, res: ServerResponse, config: D3CodeConfig, state: IdeRuntimeState, options: IdeServerOptions): Promise<void> {
  ...
  if (req.method === "GET" && url.pathname === "/api/models") {
    ...
    const key = secretRef ? await defaultSecretStore().get(secretRef) : undefined
