...
function html(): string {
  return [
    ...
    "function messageNode(message){message.createdAt=message.createdAt||Date.now();const div=document.createElement('div');if(message.kind==='tool'){div.className='tool-event '+(message.status==='running'?'running':'');div.dataset.rawText=message.text||'';div.innerHTML='<div><div class=\"tool-name\">'+escapeHtml(message.name||'D3 tool')+'</div><div class=\"tool-status\">'+escapeHtml(message.status||'done')+' · '+escapeHtml(formatTime(message.createdAt))+'</div></div><div class=\"chat-tool-result\">'+escapeHtml(message.text||'running')+'</div>';return div}div.className='chat-bubble '+(message.role||'assistant');div.innerHTML='<div class=\"message-head\"><div class=\"message-meta\">'+escapeHtml(message.role==='user'?'You':'D3 Code')+' · '+escapeHtml(formatTime(message.createdAt))+'</div></div><div class=\"message-body\"></div>'+(message.role==='assistant'?'<div class=\"message-actions\"><button class=\"copy-message\" type=\"button\" title=\"Copy response\" aria-label=\"Copy response\">'+copyIcon()+'</button></div>':'');const copy=div.querySelector('.copy-message');if(copy)copy.onclick=()=>copyText(div.dataset.rawText,copy);updateMessageNode(div,message);return div}",
