...
function html(): string {
  return [
    ...
    "</style>",
    "</head>",
    "<body data-theme=\"light\" data-view=\"chat\">",
    "<div class=\"top\"><div class=\"brand\">D3 Code IDE</div><div class=\"meta\" id=\"meta\">No profile selected</div><div class=\"view-toggle\" aria-label=\"Workspace view\"><button id=\"chatModeButton\" class=\"mode-button active\" title=\"Chat mode\" aria-label=\"Chat mode\">C</button><button id=\"editorModeButton\" class=\"mode-button\" title=\"Editor mode\" aria-label=\"Editor mode\">E</button></div><div class=\"profile-wrap\"><button id=\"profileMenuButton\" class=\"profile-button\"><span id=\"profileButtonLabel\">Profiles</span><span>v</span></button><div id=\"profileMenu\" class=\"profile-menu\"><div><div class=\"menu-title\">Profiles</div><div class=\"menu-muted\">Add, edit, remove, or select a D3 connection profile.</div></div><div id=\"profileChoices\" class=\"profile-list\"></div><button id=\"addProfile\">Add profile</button><div class=\"theme-row\" id=\"themeRow\"><button class=\"theme-choice\" data-theme-choice=\"dark\">Dark mode</button><button class=\"theme-choice active\" data-theme-choice=\"light\">Light mode</button><button class=\"theme-choice\" data-theme-choice=\"system\">System</button></div></div></div></div>",
    "<div id=\"chatView\" class=\"chat-layout\"><aside class=\"chat-sidebar\"><div class=\"dock\"><div class=\"dock-head\"><div class=\"dock-title\">D3 Workspace</div><button id=\"newChat\">New chat</button></div><div class=\"dock-body\"><div class=\"nav-section\"><div class=\"nav-kicker\">D3 map</div><div class=\"nav-row\"><span>@</span><span><b>Profile</b><span>Saved route into a D3 server/session.</span></span></div><div class=\"nav-row\"><span>A</span><span><b>Account</b><span>The application/database context, like DM or MultiSnack.</span></span></div><div class=\"nav-row\"><span>F</span><span><b>Files</b><span>D-pointers/Q-pointers, records, dictionaries, BASIC.</span></span></div><div class=\"nav-row\"><span>B</span><span><b>BASIC</b><span>Programs and cataloged subroutines that drive behavior.</span></span></div></div><div class=\"nav-section\"><div class=\"nav-kicker\">Chats</div><div id=\"chatList\" class=\"chat-list\"></div></div></div></div></aside><main class=\"chat-main\"><div class=\"dock-head\"><div><div class=\"dock-title\">D3 Code Chat</div><div class=\"dock-sub\" id=\"chatSubtitle\">No profile selected</div></div><div class=\"dock-sub\">Ask in plain English</div></div><div id=\"chatOutput\" class=\"chat-thread-main\"></div><div class=\"chat-composer-main\"><textarea id=\"chatPrompt\" placeholder=\"Ask about accounts, MultiSnack, files, dictionaries, BASIC, or what to inspect next...\"></textarea><button class=\"primary\" id=\"chatSend\">Send</button></div></main><aside class=\"chat-info\"><div class=\"dock\"><div class=\"dock-head\"><div class=\"dock-title\">Info</div><div class=\"dock-sub\" id=\"chatInfoSafety\">Safety: ask</div></div><div class=\"dock-body\"><div class=\"info-card\"><h3>Connection</h3><div class=\"info-line\"><span>Profile</span><strong id=\"chatInfoProfile\">none</strong></div><div class=\"info-line\"><span>Account</span><strong id=\"chatInfoAccount\">none</strong></div><div class=\"info-line\"><span>Model</span><strong id=\"chatInfoModel\">unknown</strong></div></div><div class=\"info-card\"><h3>D3 terms</h3><div class=\"info-copy\">A profile is the connection. An account is the D3 application/database context. MultiSnack should be treated as an account or app estate until D3 evidence says otherwise.</div></div><div class=\"info-card\"><h3>Last tool</h3><div id=\"chatLastTool\" class=\"info-copy\">No D3 tool has run in this browser session.</div></div></div></div></aside></div>",
    ...
    "let chatSessions=[];",
    "let activeChatId='';",
    "function makeChat(){const id='chat-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,7);return {id,title:'New D3 chat',updatedAt:Date.now(),messages:[]}}",
    ...
    "function recordChatMessage(message){const chat=currentChat();chat.messages.push(message);chat.updatedAt=Date.now();const firstUser=chat.messages.find(m=>m.role==='user');if(firstUser)chat.title=previewText(firstUser.text).slice(0,44)||'D3 chat';chatSessions=[chat,...chatSessions.filter(c=>c.id!==chat.id)];saveChats();renderChatList()}",
    "function messageNode(message){const div=document.createElement('div');if(message.kind==='tool'){div.className='tool-event';div.innerHTML='<div class=\"tool-name\">'+escapeHtml(message.name||'D3 tool')+'</div><div class=\"chat-tool-result\">'+escapeHtml(message.text||'running')+'</div>';return div}div.className='chat-bubble '+(message.role||'assistant');div.textContent=message.text||'';return div}",
    ...
    "function addToolEvent(t){$('chatLastTool').textContent=(t.name||'D3 tool')+': '+previewText(t.result||t.compact||t.reason||'running');return appendMessage({kind:'tool',role:'tool',name:t.name||'D3 tool',text:t.result||t.compact||t.reason||'running'})}",
    "function renderChatList(){if(!$('chatList'))return;$('chatList').innerHTML=chatSessions.map(c=>{const last=c.messages[c.messages.length-1];return `<button class=\"chat-item ${c.id===activeChatId?'active':''}\" data-chat-id=\"${escapeHtml(c.id)}\"><b>${escapeHtml(c.title||'D3 chat')}</b><span>${escapeHtml(last?previewText(last.text):'No messages yet')}</span></button>`}).join('');document.querySelectorAll('[data-chat-id]').forEach(btn=>btn.onclick=async()=>{activeChatId=btn.dataset.chatId;renderChatList();renderActiveChat();try{await api('/api/agent/reset',{method:'POST'})}catch{}})}",
    "function renderActiveChat(){const out=$('chatOutput');if(!out)return;out.innerHTML='';const chat=currentChat();if(chat.messages.length===0){const intro=document.createElement('div');intro.className='chat-bubble assistant';intro.textContent='Tell me what you want to understand about this D3 estate. Start with things like: what account am I in, is MultiSnack an account, list the files, explain this BASIC program, or map the data structure.';out.appendChild(intro);return}chat.messages.forEach(m=>out.appendChild(messageNode(m)));out.scrollTop=out.scrollHeight}",
    ...
    "async function boot(){const status=await api('/api/status');selectedProfile=status.profile||'';const account=status.account||'none';$('meta').textContent=status.profile?`Profile ${status.profile} / Account ${account}`:'No D3 profile selected';$('profileButtonLabel').textContent=status.profile||'Profiles';$('agentSafety').textContent=`Safety: ${status.safety||'ask'} / Mode: ${status.mode||'chat'}`;$('chatSubtitle').textContent=status.profile?`Profile ${status.profile} / Account ${account}`:'No D3 profile selected';$('chatInfoSafety').textContent=`Safety: ${status.safety||'ask'}`;$('chatInfoProfile').textContent=status.profile||'none';$('chatInfoAccount').textContent=account;$('chatInfoModel').textContent=status.model||'unknown';const profiles=await api('/api/profiles');profileCache=profiles.profiles||[];renderProfiles(status.profile||'');renderChatList();renderActiveChat()}",
