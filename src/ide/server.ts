...
function html(): string {
  return [
    ...
    "<div id=\"chatView\" class=\"chat-layout\"><aside class=\"chat-sidebar\"><div class=\"dock\"><div class=\"dock-head\"><div class=\"dock-title\">Session</div><button id=\"newChat\">New chat</button></div><div class=\"dock-body\"><div class=\"nav-section\"><div class=\"nav-kicker\">Chats</div><div id=\"chatList\" class=\"chat-list\"></div></div></div></div></aside><main class=\"chat-main\"><div class=\"dock-head\"><div><div class=\"dock-title\">Current chat</div><div class=\"dock-sub\" id=\"chatSubtitle\"></div></div><div id=\"runStatus\" class=\"run-status\" data-kind=\"ready\"></div></div><div id=\"chatOutput\" class=\"chat-thread-main\"></div><div class=\"chat-composer-main\"><textarea id=\"chatPrompt\" placeholder=\"Ask about accounts, files, dictionaries, BASIC, or what to inspect next...\"></textarea><button class=\"primary\" id=\"chatSend\">Send</button></div></main><aside class=\"chat-info\"><div class=\"dock\"><div class=\"dock-head\"><div class=\"dock-title\">Info</div><div class=\"dock-sub\" id=\"chatInfoSafety\"></div></div><div class=\"dock-body\"><div class=\"info-card\"><h3>Connection</h3><div class=\"info-line\"><span>Profile</span><strong id=\"chatInfoProfile\">none</strong></div><div class=\"info-line\"><span>Account</span><strong id=\"chatInfoAccount\">none</strong></div><div class=\"info-line\"><span>Model</span><strong id=\"chatInfoModel\">unknown</strong></div><div class=\"info-line\"><span>Safety</span><strong id=\"chatInfoSafetyValue\">ask</strong></div></div><div class=\"info-card\"><h3>Last tool</h3><div id=\"chatLastTool\" class=\"info-copy\">No D3 tool has run in this browser session.</div></div></div></div></aside></div>",
    "<div id=\"terminalView\" class=\"chat-layout\"><aside class=\"chat-sidebar\"><div class=\"dock\"><div class=\"dock-head\"><div class=\"dock-title\">Session</div><button id=\"newTerminalChat\">New chat</button></div><div class=\"dock-body\"><div class=\"nav-section\"><div class=\"nav-kicker\">Chats</div><div id=\"terminalChatList\" class=\"chat-list\"></div></div></div></div></aside><main class=\"terminal-main\"><div class=\"dock-head\"><div><div class=\"dock-title\">D3 Terminal</div><div class=\"dock-sub\">TCL session</div></div></div><div class=\"terminal-row\" style=\"padding:12px;border-bottom:1px solid var(--line);margin:0;background:var(--panel2)\"><input id=\"terminalCommand\" placeholder=\"WHO, LIST MD, CT CUSTOMERS 100\" /><button class=\"primary\" id=\"terminalSend\">Send</button></div><div class=\"dock-body\"><label class=\"check\"><input id=\"terminalConfirm\" type=\"checkbox\" /> confirm mutation-risk runtime command</label><div id=\"terminalMain\" class=\"terminal\" tabindex=\"0\" role=\"textbox\" aria-label=\"D3 runtime terminal\"></div></div></main><aside class=\"chat-info\"><div class=\"dock agent-shell\"><div class=\"dock-head\"><div class=\"dock-title\">Current chat</div><div class=\"dock-sub\" id=\"terminalAgentSafety\"></div></div><div class=\"dock-body agent-chat\"><div id=\"terminalAgentOutput\" class=\"agent-thread\"></div><div class=\"agent-composer\"><textarea id=\"terminalAgentPrompt\" class=\"agentbox\" placeholder=\"Message D3 Code...\"></textarea><div class=\"agent-actions\"><button class=\"primary\" id=\"terminalAgentSend\">Send</button></div></div></div></div></aside></div>",
    ...
    "</main>",
    "<section class=\"right\"><div class=\"dock agent-shell\"><div class=\"dock-head\"><div class=\"dock-title\">Current chat</div><div class=\"dock-sub\" id=\"agentSafety\"></div></div><div class=\"dock-body agent-chat\"><div id=\"agentOutput\" class=\"agent-thread\"></div><div class=\"agent-composer\"><textarea id=\"agentPrompt\" class=\"agentbox\" placeholder=\"Message D3 Code...\"></textarea><div class=\"agent-actions\"><button class=\"primary\" id=\"agentSend\">Send</button></div></div></div></div></section>",
    ...
    "async function boot(){const status=await api('/api/status');selectedProfile=status.profile||'';const account=status.account||'none';$('meta').textContent='Current chat';$('profileButtonLabel').textContent=status.profile||'Profiles';$('agentSafety').textContent='';$('terminalAgentSafety').textContent='';$('chatSubtitle').textContent='';$('chatInfoSafety').textContent='';$('chatInfoProfile').textContent=status.profile||'none';$('chatInfoAccount').textContent=account;$('chatInfoModel').textContent=status.model||'unknown';$('chatInfoSafetyValue').textContent=status.safety||'ask';const profiles=await api('/api/profiles');profileCache=profiles.profiles||[];renderProfiles(status.profile||'');renderChatList();renderActiveChat();renderTerminal()}",
    ...
    "$('terminalSend').onclick=async()=>sendTerminalCommand();",
    "$('command').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();sendTerminalCommand()}});",
    "$('terminalCommand').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();sendTerminalCommand()}});",
    "$('command').addEventListener('input',()=>{terminalDraft=$('command').value;renderTerminal()});",
    "$('terminalCommand').addEventListener('input',()=>{terminalDraft=$('terminalCommand').value;renderTerminal()});",
