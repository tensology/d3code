...
export async function runMockAcceptance(): Promise<AcceptanceReport> {
  ...
  const steps = [
    ...
    step("bundle-refresh-evidence", refreshed.written.some((file) => file.endsWith("goal-evidence.md")) && refreshed.written.some((file) => file.endsWith("completion-audit.md")) && refreshed.written.some((file) => file.endsWith("release-report.md")) && refreshed.written.some((file) => file.endsWith("proof-dashboard.json")), [`written:${refreshed.written.length}`]),
